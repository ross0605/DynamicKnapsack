...
public class LagrangianRelaxation {
    ...
    public static double [] SOLUTION = new double [Constants.getDataSize()];
    public static double [] VALUES = new double[Constants.getDataSize()];
    ...
    {
        double [] solution = new double[Constants.getDataSize()];
        KNode knapsack = new KNode(new BitSet(Constants.getDataSize()), 0);
    ...
    {
        double value = lambda * (double)Constants.getKSCapacity();
        for(int i=0; i<Constants.getDataSize(); i++)
        {
            if(SOLUTION[i] > 0)
            {
    ...
    public static double calculateValue(BitSet set, int index)
    {
        ...
        double value1 = 0;
        for(int i=size; i<Constants.getDataSize();i++)
        {
            if(SOLUTION[i] > 0)
            {
                value1+=SOLUTION[i];
